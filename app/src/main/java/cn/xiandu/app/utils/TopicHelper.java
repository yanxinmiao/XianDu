...
public class TopicHelper {
    ...
    private static TopicHelper mAllTopic;
    ...
    public List<TopicModel> getAllTopics() {
        if (mAllTopics == null || mAllTopics.size() == 0) {
            String topics = SharedPreferenceUtils.getString(MODEL_KEY);
            if (TextUtils.isEmpty(topics)){
                ...
                for (int i = 0; i < cacheStr.length; i++) {
                    ...
                    SharedPreferenceUtils.setString(MODEL_KEY, new Gson().toJson(mAllTopics));
                }
            }else{
                Type t = new TypeToken<List<TopicModel>>() {}.getType();
                List<TopicModel> tempModels = new Gson().fromJson(topics, t);
    ...
    public void clearCache(Context context) {
        SharedPreferenceUtils.clearString(MODEL_KEY);
        mAllTopics.clear();
    }
    public void saveTopics(Context context, List<TopicModel> list) {
        SharedPreferenceUtils.setString(MODEL_KEY, new Gson().toJson(list));
        mAllTopics.clear();
